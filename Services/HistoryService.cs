...
namespace TagForge.Services
{
    public class HistoryService
    {
        ...
        public HistoryService()
        {
            ...
            _appDirectory = Path.Combine(userProfile, ".tagforge");
        ...
        public async Task<HistoryBackupModel> CreateBackupAsync()
        {
            var backup = new HistoryBackupModel();
            
            // Load Indices
            backup.ChatIndex = await LoadSessionsIndexAsync("chat");
            backup.GeneratorIndex = await LoadSessionsIndexAsync("generator");
            
            // Load Chat Sessions
            foreach (var session in backup.ChatIndex)
            {
                var messages = await LoadSessionMessagesAsync(session.Id);
                if (messages.Any()) backup.ChatSessions[session.Id] = messages;
            }
            
            // Load Generator Sessions
            foreach (var session in backup.GeneratorIndex)
            {
                var messages = await LoadSessionMessagesAsync(session.Id);
                if (messages.Any()) backup.GeneratorSessions[session.Id] = messages;
            }
            
            return backup;
        }

        public async Task ImportBackupAsync(HistoryBackupModel backup)
        {
            // 1. Merge Indices
            var existingChatIndex = await LoadSessionsIndexAsync("chat");
            foreach (var newSession in backup.ChatIndex)
            {
                if (existingChatIndex.All(s => s.Id != newSession.Id))
                    existingChatIndex.Add(newSession);
            }
            await SaveSessionIndexAsync(existingChatIndex, "chat");
            
            var existingGenIndex = await LoadSessionsIndexAsync("generator");
            foreach (var newSession in backup.GeneratorIndex)
            {
                if (existingGenIndex.All(s => s.Id != newSession.Id))
                    existingGenIndex.Add(newSession);
            }
            await SaveSessionIndexAsync(existingGenIndex, "generator");
            
            // 2. Save Session Files
            foreach (var entry in backup.ChatSessions)
            {
                await SaveSessionAsync(entry.Key, entry.Value, "chat");
            }
            
            foreach (var entry in backup.GeneratorSessions)
            {
                await SaveSessionAsync(entry.Key, entry.Value, "generator");
            }
        }
    ...
    public class HistoryBackupModel
    {
        public List<ChatSession> ChatIndex { get; set; } = [];
        public List<ChatSession> GeneratorIndex { get; set; } = [];
        public Dictionary<string, List<ChatMessage>> ChatSessions { get; set; } = [];
        public Dictionary<string, List<ChatMessage>> GeneratorSessions { get; set; } = [];
    }
}
