...
namespace TagForge.Services.Providers
{
    public class CerebrasProvider : IAIProvider
    {
        ...
        private const string DefaultBaseUrl = "https://api.cerebras.ai/v1/chat/completions";
        private const string ModelsUrl = "https://api.cerebras.ai/v1/models";
        ...
        public async Task<string> GenerateAsync(string systemPrompt, string userPrompt, string model, string apiKey, string baseUrl)
        {
            ...
            var payload = new
            {
                model = string.IsNullOrEmpty(model) ? "llama3-70b-8192" : model, // Example valid model for Cerebras
                messages = messages
            };
            ...
            if (!response.IsSuccessful)
            {
                throw new Exception($"Cerebras API Error: {response.Content}");
            }
        ...
        public async Task<List<string>> FetchModelsAsync(string apiKey, string baseUrl)
        {
            ...
            return models.Count > 0 ? models : new List<string> { "llama3-8b-8192", "llama3-70b-8192" };
        }
        public async IAsyncEnumerable<string> GenerateStreamingAsync(string systemPrompt, string userPrompt, string model, string apiKey, string baseUrl)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                 ...
                 throw new Exception($"Cerebras API Error: {err}");
            }
